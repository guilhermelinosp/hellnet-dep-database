...
internal sealed class NpgsqlExecutor : IDatabaseExecutor
{
    private readonly NpgsqlDataSource _dataSource;
    private readonly HellnetDatabaseOptions _options;
    private readonly ILogger<NpgsqlExecutor> _logger;

    public NpgsqlExecutor(
        NpgsqlDataSource dataSource,
        HellnetDatabaseOptions options,
        ILogger<NpgsqlExecutor> logger)
    {
    ...
    {
        await using var conn = await _dataSource.OpenConnectionAsync(ct);
        var result = await conn.QueryAsync<T>(new CommandDefinition(sql, parameters,
            commandTimeout: _options.CommandTimeoutSeconds, cancellationToken: ct));
